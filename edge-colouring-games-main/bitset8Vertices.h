...
#define unsafeNext(set, current) (__builtin_clzll(set) + (current) >= 8 ? -1 : (current) + __builtin_ctzll((set) >> ((current) + 1)) + 1)
...
#define complement(set, sizeOfUniverse) (~(set) << (8-(sizeOfUniverse)) >> (8-(sizeOfUniverse)))
