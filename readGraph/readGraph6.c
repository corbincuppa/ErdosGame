...
int loadGraph(const char * graphString, int numberOfVertices, bitset* edgeList) {
		...
		for (i = prev(graphString[index] - 63, 6); i != -1;
		 i = prev(graphString[index] - 63, i)) {
			...
			if(currentVertex > neighbour){
				*edgeList |= singleton(currentVertex*(currentVertex-1)/2 + neighbour);
			}
			else{
				*edgeList |= singleton(neighbour*(neighbour-1)/2 + currentVertex);
			}
		}
	}
	if(finalChar == '\0') {
